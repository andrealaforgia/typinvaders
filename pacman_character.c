...
static sprite_t select_frame(const pacman_character_ptr p) {
    if (p->mouth_phase == 0)
...
void pacman_character_destroy(pacman_character_ptr p) { (void)p; }
...
void pacman_character_render(const pacman_character_ptr p,
    ...
    point_t pos = pacman_character_position(p);
